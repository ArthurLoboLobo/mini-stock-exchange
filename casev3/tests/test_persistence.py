...
class TestEviction:
    @pytest.mark.asyncio
    async def test_closed_orders_retained_in_engine(self, session_factory, two_brokers, test_engine):
        """V3: closed orders are NOT evicted — they stay in memory for reads."""
        broker_a, _ = two_brokers
        now = datetime.now(timezone.utc)
        order_id = uuid.uuid4()
        ...
        # V3: Order should still be in engine.orders (no eviction)
