...
class TestPersistenceVisibility:
    """V3: orders are served from memory immediately. DB is async backup."""

    async def test_order_visible_immediately_from_memory(self, client: AsyncClient, broker_with_key):
        """Submit order, don't flush -> GET returns 200 (served from memory)."""
        _, key = broker_with_key
        ...
        # V3: order is in memory, GET returns 200 immediately
        get_resp = await client.get(f"/orders/{order_id}", headers=auth_header(key))
        assert get_resp.status_code == 200

    async def test_order_data_correct_from_memory(self, client: AsyncClient, broker_with_key):
        """Submit order -> GET returns correct data from memory."""
        _, key = broker_with_key
    ...
    async def test_balance_updates_after_trade(
        ...
        """Two brokers trade -> balances updated immediately in memory."""
        ...
        # V3: no flush needed — balances updated in memory immediately
