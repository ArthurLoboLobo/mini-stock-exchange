...
- `perf_realistic.py` — Simulated trading window with open-loop Poisson dispatch, Zipf symbol distribution, coordinated omission correction, cancel orders with async Future-based order ID resolution, and symbol-based worker assignment. B3-calibrated constants (450 symbols, 100 brokers, 700 orders/sec, 245 cancels/sec). Uses `aiohttp` + `uvloop` + multiprocessing to avoid client-side bottlenecks. Configurable via `--scale`, `--duration`, `--workers`.
