...
Mini Stock Exchange V2 — a Python/FastAPI order matching engine. V2 matches orders **in-memory** (SortedDict order book) and persists results **asynchronously** to PostgreSQL in batches every ~30ms. This replaces V1's synchronous DB-driven matching for dramatically lower latency.

Same API surface as V1. Same matching rules. Different architecture.
...
### In-memory matching + async persistence
...
GET  /orders/{id}      → DB read (may be ~30ms stale until persistence flush)
```

The `Engine` singleton (`app/engine/__init__.py`) holds all matching state:
- `orders: dict[UUID, Order]` — open orders only (evicted after DB write confirms closure)
...
`flush_batch()` writes one transaction: INSERT orders → INSERT trades → UPDATE order statuses → UPDATE broker balances. After commit: fire webhooks, evict closed orders from memory.

### Startup recovery (`app/main.py` lifespan)
1. Load broker auth hashes into `engine.brokers_by_key_hash`
2. Load open non-expired orders into `engine.orders` + `engine.book` (FIFO order preserved)
3. Start persistence loop background task
...
**Option A** (most tests): `await flush_persistence()` manually drains the queue and calls `flush_batch()`. Deterministic — no background task involved.
...
- `clean_state` — per-test: disposes stale DB pool, cancels orphaned persistence tasks, truncates tables, resets engine memory, sets admin key
- `client` — `httpx.AsyncClient` via `ASGITransport` (depends on `clean_state`)
- `broker_with_key` / `second_broker_with_key` — register broker via API, return `(broker_id, api_key)`
...
- `app/main.py` — FastAPI app, lifespan (broker loading, order recovery, persistence loop start)
...
- `app/engine/__init__.py` — `Engine` singleton (orders, book, brokers_by_key_hash, queue, persistence_task)
...
- `app/engine/persistence.py` — Queue items (`NewOrderItem`, `TradeItem`, `OrderUpdateItem`), `flush_batch()`, `run_persistence_loop()`
- `app/routers/orders.py` — POST/GET/cancel order endpoints (`POST /orders`, `GET /orders/{id}`, `POST /orders/{id}/cancel` returns 204 No Content with no body, pure in-memory with no DB dependency — non-existent/already-closed orders are silent no-ops)
