...
async def flush_batch(
    ...
    async with session_factory() as session:
        ...
        # Evict closed orders from engine.orders
        for u in order_updates.values():
            if u.status == OrderStatus.closed:
                engine.orders.pop(u.order_id, None)

        # Fire webhooks for committed trades
        if trades and trade_broker_ids:
            result = await session.execute(
                text("SELECT id, webhook_url FROM brokers WHERE id = ANY(:ids)"),
                {"ids": list(str(bid) for bid in trade_broker_ids)},
            )
